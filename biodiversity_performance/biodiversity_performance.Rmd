...
The dataset comes from two csv large files (4G, 2.4G) which are can not open with an ordinary computer.
...
![Screenshot of biodiversity](screenshot.png){width=50%}
...
We can compare multiple ways to filter our data. Here we tried three methods (`grepl`, `%in`, `str_detect`) and we will select the faster one.
...
Adding `%>% bindCache(vals$countries, cache = "session")` the app cache to the memory all processes of `renderleaflet()` for a specific `vals$countries`. If the user select a new `vals$countries`, le computing will be done, if not the app return saved cache. Concretely, we do not view progress bar for loading, preprocessing and ploting if the app was cached the `vals$countries`.
...
```{r, eval=FALSE}
  ...
  tic(paste0("Loading Map for ", countries_id))
...
# Generic key skills {.tabset .tabset-fade .tabset-pills}
