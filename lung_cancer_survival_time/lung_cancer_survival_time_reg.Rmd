---
title: "Survival Time prediction of  Lung Cancer With Python & R"
subtitle: | 
          ...
          | - lifelines module: CoxPHFitter, WeibullAFTFitter , LogNormalAFTFitter, LogLogisticAFTFitter 
...
train_8_sparse <- sparse.model.matrix(SurvivalTime ~., data=train_8 %>% select(-Event))
dtrain_8 <- xgb.DMatrix(data=train_8_sparse, label = train_8$SurvivalTime)

options(na.action='na.pass')
valid_2_sparse <- sparse.model.matrix(SurvivalTime ~., data=valid_2%>% select(-Event))
...
params <- list(booster = "gblinear",
              ...
              eval_metric = "rmse",
              base_score=1,
              n_estimators=1000 #  for Binary classification error rate
