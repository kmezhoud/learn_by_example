...
# Fit ARIMA model
arima_fit = auto_arima(trend, seasonal=True, m=7, stepwise=False, trace=True) #, alpha=[0.5, 0.95]
...
# Assuming 'arima_model' is the fitted ARIMA model
# pred_len is the length of the forecast horizon

# Generate forecasts
arima_forecast, conf_int = arima_fit.predict(n_periods=pred_len, return_conf_int=True)

# 'arima_forecast' will contain the point forecasts
# 'conf_int' will contain the confidence intervals

# Convert the forecasts to a DataFrame with appropriate dates
# Assuming 'split_date' is available
...
# Assuming 'arima_forecast_df' contains the forecasted values and confidence intervals

# Plot the ARIMA forecast
plt.figure(figsize=(10, 6))
plt.plot(arima_forecast_df.index, arima_forecast_df['forecast'], label='ARIMA Forecast', color='blue')

# Plot confidence intervals
plt.fill_between(arima_forecast_df.shape[0], arima_forecast_df['lower_conf_int'], arima_forecast_df['upper_conf_int'], color='lightblue')
...
plt.figure(figsize=(10, 6))
plt.plot(arima_forecast_df.index, arima_forecast_df['forecast'], label='ARIMA Predictions', color='blue')

# Overlay the actual values from 'visits_valid'
plt.plot(visits_valid.index, visits_valid['visitors'], label='Actual Values', color='grey', linestyle='--')

# Customize the plot
plt.xlabel("Time [weeks]")
plt.ylabel("log1p visitors vs ARIMA predictions")
plt.legend('')
plt.show()
...
x_axis = np.arange(train.shape[0] + arima_forecast_df.shape[0])
axes[0].plot(x_axis[:train.shape[0]], train, alpha=0.75)
axes[0].scatter(x_axis[train.shape[0]:], arima_forecast_df, alpha=0.4, marker='o')
axes[0].scatter(x_axis[test.shape[0]:], test, alpha=0.4, marker='x')
axes[0].fill_between(x_axis[-conf_int.shape[0]:], conf_int[:, 0], conf_int[:, 1],
                     alpha=0.1, color='b')
plt.show()
```
...
arima = pm.auto_arima(train, start_p=1, start_q=1, d=0, max_p=5, max_q=5,
                      out_of_sample_size=10, suppress_warnings=True, alpha=[0.5, 0.95],
