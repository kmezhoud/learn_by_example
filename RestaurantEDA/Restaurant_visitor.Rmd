...
p2 = sns.FacetGrid(p2_data, row='wday', hue="wday", palette='viridis',
                 aspect=15, height=.5)
p2.map(sns.kdeplot, 'log1p_visitors', bw_adjust=.5, clip_on=False, fill=True, alpha=1, linewidth=.5, ax=ax1)
p2.map(sns.kdeplot, 'log1p_visitors', clip_on=False, color="black", lw=2, bw_adjust=.5, ax=ax1)
p2.set(xlim=(0, 10))
...
p2.map(label, 'log1p_visitors')

p2.set_titles("")
p2.set(yticks=[], ylabel="")
p2.despine(bottom=True, left=True)
p2.fig.subplots_adjust(hspace=-.25)
...
p4 = sns.FacetGrid(p4_data, row='month', hue="month", palette='viridis',
                 aspect=15, height=.5)
p4.map(sns.kdeplot, 'log1p_visitors', bw_adjust=.5, clip_on=False, fill=True, alpha=1, linewidth=.5, ax=ax2)
p4.map(sns.kdeplot, 'log1p_visitors', clip_on=False, color="black", lw=2, bw_adjust=.5, ax=ax2)
p4.set(xlim=(0, 10))

p4.map(label, 'log1p_visitors')

p4.set_titles("")
p4.set(yticks=[], ylabel="")
p4.despine(bottom=True, left=True)
p4.fig.subplots_adjust(hspace=-.25)
...
sns.boxplot(data=all_reserve, x='wday', y='visitors_minus_reserve', 
             hue='wday', palette='viridis')
plt.xlabel('Weekday')
plt.ylabel('Visitors - Reserve Visitors')
...
sns.regplot(data=area_visits, x='air_count', y='mean', scatter=False, color='black', ax=ax3)
...
# Create a base map
m = folium.Map(location=[foo['latitude'].mean(), foo['longitude'].mean()], zoom_start=5)
...
air_store.apply(add_air_marker, axis=1)

# Define a function to add a marker for med_coord_air
def add_center_marker(row):
    folium.Marker(location=[row['latitude'], row['longitude']],
                  popup="Center Point",
...
med_coord_air.apply(add_center_marker)
